...
using Game.State;
using HarmonyLib;
using InterchangedIndustryUnloaderMod;
...
namespace Model.Ops
{
    public class InterchangedIndustryUnloader : IndustryComponent
    {
        ...
        public void ServeInterchange(IIndustryContext ctx, Interchange interchange)
        {
            ...
            int revenueTally = 0;
            int carCount = 0;
            InterchangedIndustryUnloaderPlugin plugin = SingletonPluginBase<InterchangedIndustryUnloaderPlugin>.Shared ?? throw new ArgumentNullException();
            int[] minMaxReturnHours = { 23, 23 };
            foreach (IOpsCar car in list)
            {
                int returnHours = 23;

                if (plugin.UseRandomReturnTimes)
                {
                    returnHours = new System.Random().Next(plugin.MinHours, plugin.MaxHours);
                    minMaxReturnHours[0] = Math.Min(minMaxReturnHours[0], returnHours);
                    minMaxReturnHours[1] = Math.Max(minMaxReturnHours[1], returnHours);
                }
                GameDateTime returnTime = ctx.Now.AddingHours(returnHours);

                (float quantity, float capacity) = car.QuantityOfLoad(load);
                if (quantity < capacity)
                {
                    Debug.LogWarning("Car ID: " + car.Id + ", name: " + car.DisplayName + " isn't actually full, but we'll take it anyway.");
                }
                int revenueCar = Mathf.RoundToInt(quantity * load.payPerQuantity * plugin.PayPercentage);
                if (revenueCar > 0)
                {
                    revenueTally += revenueCar;
                    carCount++;
                }
                car.Unload(load, quantity);
                Debug.Log("Car ID: " + car.Id + ", name: " + car.DisplayName + " with capacity of " + capacity + " unloaded " + quantity + " of " + load.description + " for $" + revenueCar  + ". Current tally: $" + revenueTally);
            ...
            if (revenueTally > 0)
            {
                base.Industry.ApplyToBalance(revenueTally, ledgerCategory, null, carCount, quiet: true);
                string returnTimeString = "";

                if(minMaxReturnHours[0] == minMaxReturnHours[1])
                {
                    returnTimeString = minMaxReturnHours[0] >= 12 ? ((minMaxReturnHours[0] + 12) / 24).Pluralize("day") : minMaxReturnHours[0].Pluralize("hour");
                }
                else
                {
                    returnTimeString = String.Concat(
                        minMaxReturnHours[0] >= 12 ? ((minMaxReturnHours[0] + 12) / 24).Pluralize("day") : minMaxReturnHours[0].Pluralize("hour"),
                        // minMaxReturnHours[0].Pluralize("hour"),
                        " to ",
                        minMaxReturnHours[1] >= 12 ? ((minMaxReturnHours[1] + 12) / 24).Pluralize("day") : minMaxReturnHours[1].Pluralize("hour")
                        // minMaxReturnHours[1].Pluralize("hour")
                    );
                }
                Multiplayer.Broadcast(string.Format("Sold {0} of {1} at {2} for {3:C0}. Expected return: {4}.", carCount.Pluralize("car"), load.description, HyperlinkToThis, revenueTally, returnTimeString));
            }
        }
        ...
        public override void BuildPanel(UIPanelBuilder builder)
        {
            builder.AddSection("Via Interchange: " + base.name + " - " + load.description, delegate (UIPanelBuilder uIPanelBuilder)
            {
                ...
                if (load.units == LoadUnits.Quantity)
                {
                    uIPanelBuilder.AddField("Price", $"{Mathf.RoundToInt(nominalQuantityPerCarLoad * load.payPerQuantity):C0} per {load.QuantityString(nominalQuantityPerCarLoad)}");
                }
                else
                {
...
namespace InterchangedIndustryUnloaderMod
{
    public class InterchangedIndustryUnloaderPlugin : SingletonPluginBase<InterchangedIndustryUnloaderPlugin>, IModTabHandler
    {
        // Lazy settings adding
        public bool UseRandomReturnTimes { get; private set; } = false;
        public int MinHours { get; private set; } = 12;
        public int MaxHours { get; private set; } = 168;
        public float PayPercentage { get; private set; } = 1.0f;
        ...
        {
            builder.AddField("Randomize Return Times", UIPanelBuilderCompatibility.AddToggleCompat(builder, (Func<bool>)(() => UseRandomReturnTimes), (Action<bool>)delegate (bool value)
            {
                UseRandomReturnTimes = value;
                builder.Rebuild();
            }, true));
            if (UseRandomReturnTimes)
            {
                builder.AddField("Minimum Hours to Return", builder.AddSlider(() => MinHours, () => $"{MinHours}", delegate (float setting)
                {
                    MinHours = Math.Min((int)setting, MaxHours);
                }, 0, 168, true));
                builder.AddField("Maximum Hours to Return", builder.AddSlider(() => MaxHours, () => $"{MaxHours}", delegate (float setting)
                {
                    MaxHours = Math.Max((int)setting, MinHours);
                }, 0, 168, true));
            }
            builder.AddField("Pay multiplier %", builder.AddSlider(() => PayPercentage*100, () => $"{PayPercentage*100:0}%", delegate (float setting)
            {
                PayPercentage = setting / 100f;
            }, 0, 200, true));
        }
